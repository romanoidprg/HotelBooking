...
public enum DoneActiveOrderCommand implements Command {
    INSTANCE;

    private static final ResponseContext DONE_ORDER = new ResponseContext() {
        @Override
        public String getPage() {
            return Pages.DONE_ORDER.page;
        }
    ...
    public ResponseContext execute(RequestContext req) {
        ...
        if (orderDao.setStatus(orderId, OrderStatus.DONE)) {
            List<Order> orderList = (List<Order>) req.getSession().getAttribute(Vars.ORDER_LIST.var);

            orderList.stream().filter(o -> o.getId() == orderId).findFirst().get().setStatus(OrderStatus.DONE);
